...
```bash
PROJECT_ID=project-c2a014a9-0b24-44a9-abb ./scripts/gcp-vm-preflight.sh
```

Create a budget alert before creating the VM:

```bash
PROJECT_ID=project-c2a014a9-0b24-44a9-abb ./scripts/gcp-budget-alert.sh
```

Then deploy:

```bash
PROJECT_ID=project-c2a014a9-0b24-44a9-abb ./scripts/gcp-vm-create-and-deploy.sh
```
...
`verify-imports` is the live article-import acceptance check. It creates a temporary verification user, imports text and a multi-chapter EPUB through the public API, waits for blocks/tokens/progress, verifies invalid EPUBs fail with a visible processing error, and prints recent `article_processing_start` / `article_processing_ready` / `article_processing_failed` API log markers when available.
